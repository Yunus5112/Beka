          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-5xl font-bold mb-6">
                {t('services.financial.detailPage.hero.title')}
              </h1>
              <p className="text-xl text-blue-100 mb-8 leading-relaxed">
                {t('services.financial.detailPage.hero.description')}
              </p>
            ...
            <div className="relative">
              <div className="bg-white bg-opacity-10 rounded-2xl p-8 backdrop-blur-sm">
                <TrendingUp className="w-24 h-24 text-white mx-auto mb-6" />
                <h3 className="text-2xl font-bold text-center mb-4">
                  {t('services.financial.detailPage.hero.subtitle')}
                </h3>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {Array.isArray(services) && services.map((service, index) => {
              const Icon = index === 0 ? DollarSign : FileText;
              return (
                <div
                  key={index}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {Array.isArray(benefits) && benefits.map((benefit, index) => {
              const icons = [Users, TrendingUp, CheckCircle, FileText];
      ...
      <section className="bg-gradient-to-r from-[#1E272D] to-[#6B7473] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-bold text-white mb-6">
            {t('services.financial.detailPage.cta.title')}
          </h2>
          <p className="text-xl text-blue-100 mb-8 max-w-2xl mx-auto">
            {t('services.financial.detailPage.cta.description')}
          </p>
          <Link
            to="/appointment"
          ...
          >
            <Calendar className="w-5 h-5 mr-2" />
            {t('services.financial.detailPage.cta.button')}
          </Link>
        </div>
      </section>
    </div>
